...
<img src="/media/Line Following Robot/image-20220910112012012.png" alt="image-20220910112012012" style="zoom:50%;" />

<img src="/media/Line Following Robot/image-20220910112030119.png" alt="image-20220910112030119" style="zoom:67%;" />

<img src="/media/Line Following Robot/image-20220910112038287.png" alt="image-20220910112038287" style="zoom:50%;" />
...
<div  style=" float: left;">
    <img src="/media/Line Following Robot/GdSm.gif" alt="Maze GIF - Find on GIFER"  />
    <p><i>每到分岔口都向左，沿著牆壁而行</i></p>
</div>

<div  style=" float: right;">
    <img src="/media/Line Following Robot/Tremaux_Maze_Solving_Algorithm.gif" alt="投石問路"/>
    <p><i>在每個路口放下小石塊</i></p>
</div>

<div >
    <img src="/media/Line Following Robot/programming_wavefront_animation.gif" alt="wavefront"/><img src="/media/Line Following Robot/v2-c08635b03c6baf9027d62540fe855e51_b.gif" alt="路徑規劃| 圖搜尋演算法：DFS、BFS、GBFS、Dijkstra、A* - 知乎_osc_2axit9df - MdEditor" style="zoom:45%;" />
    <p><i>洪水演算法</i></p>
</div>
...
<img src="/media/Line Following Robot/image-20220910133903185.png" alt="image-20220910133903185" style="zoom:50%;" />

例如你的冷氣機控制著房內的溫度保持27攝氏度，慢煮機控制著水溫保持在54攝氏度，電鉻鐵保持尖端的溫度為320攝氏度等等。

<img src="/media/Line Following Robot/image-20220910130953397.png" alt="image-20220910130953397" style="zoom:80%;" />

又或者，常用的伺服馬達就是一個典型的控制系統例子，一般的有刷馬達，只要供電就會不停旋轉，但伺服馬達連接上電位器，能感測到現在的角度，控制並保持特定的角度。

<img src="/media/Line Following Robot/image-20220910131530775.png" alt="image-20220910131530775" style="zoom:50%;" />

利用控制系統，除了溫度和角度，也可以控制位置。

<img src="/media/Line Following Robot/ball and beam.gif" alt="ball and beam" style="width:45%;" /><img src="/media/Line Following Robot/floating magnet.gif" alt="floating magnet" style="width:45%;" />

而近年常見的自平衡車和無人機，都是控制系統的一種，分別控制其姿態(Attitude)和高度(Altitude)。

<img src="/media/Line Following Robot/0K_6SGkAljzrSUbOa.gif" alt="Fresh look at self-balancing robot algorithm | by Dominik Nowak | Husarion  Blog | Medium" style="width:45%;" /><img src="/media/Line Following Robot/233b8bd98dbf370ae8eb369537a5a642.gif" alt="3DOF Ball on Plate Using Closed Loop Stepper Motors | Stepper motor,  Electronics projects, Diy electronics" style="width:45%;" />
...
<img src="/media/Line Following Robot/image-20220910134904196.png" alt="image-20220910134904196" style="width:45%;" /><img src="/media/Line Following Robot/image-20220910135207591.png" alt="image-20220910135207591" style="width:45%;" />

但如果馬達是要雙方向轉動，即可以正轉或反轉的話，電路就比較複雜，需要用到H橋(H-bridge)來控制。

<img src="/media/Line Following Robot/h bridge.gif" alt="h bridge" style="width:80%;" />

如果要控制兩個馬達的話，就需要至少8顆三極管，線路就會變得很複雜。幸好，有市面上有專門的IC能做到相同的效果。

<img src="/media/Line Following Robot/image-20220910144040789.png" alt="image-20220910144040789" style="width:80%;" />
...
<img src="/media/Line Following Robot/image-20220910151223248.png" alt="image-20220910151223248" style="zoom: 33%;" />

**但每次控制馬達，都要打4句指令，是否覺得很麻煩呢?重覆的指令，而只改變變數的話，我們可以自定一個函數。**

在Arduino右上角，有一個三個點的符號，按下去，就會見到一個新增標籤的選項，之後輸入新標籤的檔案名稱(`function`)，就會出多一個分頁。

<img src="/media/Line Following Robot/image-20230926085500403.png" alt="image-20230926085500403" style="width:45%;" /><img src="/media/Line Following Robot/image-20230926085514646.png" alt="image-20230926085514646" style="width:45%;" />
...
<img src="/media/Line Following Robot/image-20220910154705328.png" alt="image-20220910154705328" style="width: 80%;" />

利用這些指令，我們首先測試一下馬達的接線是否正確。例如下面的程式，兩邊馬達應為***左正轉***，***右正轉***，***兩邊一起正轉***，***兩邊一起停***。
::: tip
記得打開鋰電池開關，否則USB的電是不足以同時推動兩隻馬達
:::
...
void loop() {
    motorControl(255, 0); // Left motor forward, right motor stop
    delay(3000); // Wait for 3 seconds
    motorControl(0, 255); // Left motor stop, right motor forward
    delay(3000); // Wait for 3 seconds
    motorControl(255, 255); // Both motors forward
    delay(3000); // Wait for 3 seconds
    motorControl(0, 0); // Both motors stop
    delay(3000); // Wait for 3 seconds
}
```
...
<img src="/media/Line Following Robot/Screenshot 2023-09-26 092620.png" alt="Screenshot 2023-09-26 092620" style="width:45%;" /><img src="/media/Line Following Robot/Screenshot 2023-09-26 092832.png" alt="Screenshot 2023-09-26 092832" style="width:45%;" />
...
byte IR_sensor_pin[] = {12, 11, 10, 9, 8}; // IR sensor pins //[!code ++]
boolean state[5]; // IR sensor states //[!code ++]
...
void setup() {
  ...
  Serial.begin(115200); // [!code ++]
    for (int i = 0; i <= 4; i++) // [!code ++]
        pinMode(IR_sensor_pin[i], INPUT); // [!code ++]
}

void loop() {
    motorControl(255, 0); // Left motor forward, right motor stop // [!code --]
    delay(3000); // Wait for 3 seconds // [!code --]
    motorControl(0, 255); // Left motor stop, right motor forward // [!code --]
    delay(3000); // Wait for 3 seconds // [!code --]
    motorControl(255, 255); // Both motors forward // [!code --]
    delay(3000); // Wait for 3 seconds // [!code --]
    motorControl(0, 0); // Both motors stop // [!code --]
    delay(3000); // Wait for 3 seconds // [!code --]
    // Read and store the sensors values // [!code ++]
    for (int i = 0; i <= 4; i++) // [!code ++]
        state[i] = digitalRead(IR_sensor_pin[i]); // [!code ++]
    // [!code ++]
    printSensorValue(); // [!code ++]
}
```
...
void printSensorValue() { // [!code ++]
    for (int i = 0; i <= 4; i++) // [!code ++]
        Serial.print(state[i]); // [!code ++]
    Serial.println(); // [!code ++]
} // [!code ++]
...
<img src="/media/Line Following Robot/image-20230926094544562.png" alt="image-20230926094544562" style="zoom:50%;" />
...
<img src="/media/Line Following Robot/image-20220913092619737.png" alt="image-20220913092619737" style="width: 80%;" />
...
<img src="/media/Line Following Robot/image-20220913103004470.png" alt="image-20220913103004470" style="width:50%;" />
...
<img src="/media/Line Following Robot/image-20220913103121107.png" alt="image-20220913103121107" style="width:25%;" />
...
<img src="/media/Line Following Robot/image-20221024090723698.png" alt="image-20221024090723698"  />
<!-- ![image-20221024090723698](/media/Line Following Robot/image-20221024090723698.png) -->
...
byte IR_sensor_pin[] = {12, 11, 10, 9, 8}; // IR sensor pins 
boolean state[5]; // IR sensor states 
...
void loop() {
    // Read and store the sensors values 
    for (int i = 0; i <= 4; i++) 
...
void sensor2number() { //[!code ++]
    if (state[0] == 0 && state[1] == 1 && state[2] == 1 && state[3] == 1 && state[4] == 1) input = 4.0; //[!code ++]
    if (state[0] == 0 && state[1] == 0 && state[2] == 1 && state[3] == 1 && state[4] == 1) input = 3.0;//[!code ++]
    if (state[0] == 1 && state[1] == 0 && state[2] == 1 && state[3] == 1 && state[4] == 1) input = 2.0;//[!code ++]
    if (state[0] == 1 && state[1] == 0 && state[2] == 0 && state[3] == 1 && state[4] == 1) input = 1.0;//[!code ++]
    if (state[0] == 1 && state[1] == 1 && state[2] == 0 && state[3] == 0 && state[4] == 1) input = -1.0;//[!code ++]
    if (state[0] == 1 && state[1] == 1 && state[2] == 0 && state[3] == 0 && state[4] == 1) input = -2.0;//[!code ++]
    if (state[0] == 1 && state[1] == 1 && state[2] == 1 && state[3] == 0 && state[4] == 0) input = -3.0;//[!code ++]
    if (state[0] == 1 && state[1] == 1 && state[2] == 1 && state[3] == 1 && state[4] == 0) input = -4.0;//[!code ++]
}//[!code ++]
```
:::
...
<img src="/media/Line Following Robot/image-20220913115216707.png" alt="image-20220913115216707" style="width:40%;" />
...
<img src="/media/Line Following Robot/main-qimg-4b8ac3e9b4918aeb1999b6fd45f04814.gif" alt="Why do formula 1 drivers wiggle the steering wheels? - Quora" style="width:40%;" />
...
<img src="/media/Line Following Robot/image-20220913140805350.png" alt="image-20220913140805350" style="width:50%;" />
...
<img src="/media/Line Following Robot/image-20220913144700927.png" alt="image-20220913144700927" style="width:50%;" />
...
float input; //[!code --]
float input, P_gain = 60.0, output; //[!code ++]
...
void loop() {
    ...
    //Serial.println(input); //[!code ++]
    //[!code ++]
    output = P_gain * input; //[!code ++]
    //[!code ++]
    if (input > 0) motorControl(255-output, 255); //[!code ++]
    else motorControl(255, 255-abs(output)); //[!code ++]
}
```
...
void sensor2number() { 
    ...
    if (state[0] == 1 && state[1] == 1 && state[2] == 0 && state[3] == 0 && state[4] == 1) input = -2.0;
...
<!-- <a href="https://lokchonmou.github.io/v2/interactive/lineFollowerSim/index.html" target="_blank" rel="noreferrer">Line Foloower Simulator</a>

<iframe width="100%" height="650" src="https://lokchonmou.github.io/v2/interactive/lineFollowerSim/index.html"></iframe> -->
...
<img src="/media/Line Following Robot/image-20220913153344700.png" alt="image-20220913153344700" style="zoom: 40%;" />
...
<img src="/media/Line Following Robot/image-20220914121301004.png" alt="image-20220914121301004" style="zoom:50%;" />
...
<img src="/media/Line Following Robot/Poistion-time-to-velocity-time-graph.png" alt="position time graph to velocity time graph" style="zoom:80%;" />
...
$$
<img src="/media/Line Following Robot/image-20220915110456845.png" alt="image-20220915110456845" style="zoom:80%;" />

這裡有兩個參數，分別為$K_p$和$K_d$，只要調當調試這個兩參數的值，就可以順利控制機械人。

<img src="/media/Line Following Robot/image-20220915110650112.png" alt="image-20220915110650112" style="width:80%;" />
