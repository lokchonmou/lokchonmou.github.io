...
`Construction of geometric figures/` 經核對為舊版 Rhino 教學草稿，完整內容已併入 CAD & CAM，不再作為獨立新站版塊。

`ESP32 & Vibe Coding/` 內容已搬入新站，包括文章、教學附件、搶答機與「時間的追逐」互動頁。舊來源暫時保留作對照。

`interactive/` 與 `portal/` 已整理成三組展示入口；19 個本地作品使用獨立靜態頁，外部 GitHub Pages 工具保留外連。「時間的追逐」則歸入探索與隨筆。
