...
2. 鳥的生命沒有限制，令成功生存的鳥一直生存下去，沒有機會變下一世代，不利基因遺傳，試設定一個liftcount，鳥在經過2000個frameCount後就會滅亡，還入下一個世代。
3. 在原例子中，神經網路是`NeuralNetwork(5, 8, 2)`，因為輸入的考慮因素，還有一個是`birdVec`，試著將`birdVec.y`加入變為考慮: `inputs.append( map(this.birdVec.y, -5, 5, 0, 1))`
